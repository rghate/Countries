...
class CountriesUITests: XCTestCase {
    ...
    func testRegions() {
        ...
        // The purpose of this test is to check country regions are correctly displayed,

        let australiaRegion = app.staticTexts["Australia-Region"]
        XCTAssertTrue(australiaRegion.waitForExistence(timeout: kTimeOut))
        XCTAssertEqual("Oceania", australiaRegion.label)
        
        let colombiaRegion = app.staticTexts["Colombia-Region"]
        XCTAssertTrue(colombiaRegion.waitForExistence(timeout: kTimeOut))
        XCTAssertEqual("Americas", colombiaRegion.label)

        let hongKongRegion = app.staticTexts["Hong Kong-Region"]
        XCTAssertTrue(hongKongRegion.waitForExistence(timeout: kTimeOut))
        XCTAssertEqual("Asia", hongKongRegion.label)
    }
    ...
    func testAreas() {
        ...
        // The purpose of this test is to check area of the countries are correct,
        // and that we don't try and display non-existent areas, such as for French Guiana.
        
        let arubaArea = app.staticTexts["Aruba-Area"]
        XCTAssertTrue(arubaArea.waitForExistence(timeout: kTimeOut))
        XCTAssertEqual("180 km²", arubaArea.label)
        
        let canadaArea = app.staticTexts["Canada-Area"]
        XCTAssertTrue(canadaArea.waitForExistence(timeout: kTimeOut))
        XCTAssertEqual("9,984,670 km²", canadaArea.label)

        let frenchGuianaArea = app.staticTexts["French Guiana-Area"]
        XCTAssertFalse(frenchGuianaArea.waitForExistence(timeout: kTimeOut))
    }
    ...
    func testAreaFormat() {
        ...
        // The purpose of this test is to check that the area is formatted with comma separators.
        //   1,000,000 <= Afghan area <= 999,999,999.
        //   1,000,000,000 <= China area <= 999,999,999,999
        //   1,000 <= Antarctica area <= 9,999
        
        let afghanArea = app.staticTexts["Afghanistan-Area"]
        XCTAssertTrue(afghanArea.waitForExistence(timeout: kTimeOut))
        var commas = afghanArea.label.filter({return $0 == ","}).count
        XCTAssertEqual(1, commas)
        
        let chinaArea = app.staticTexts["China-Area"]
        XCTAssertTrue(chinaArea.waitForExistence(timeout: kTimeOut))
        commas = chinaArea.label.filter({return $0 == ","}).count
        XCTAssertEqual(2, commas)
        
        let antarcticaArea = app.staticTexts["Antarctica-Area"]
        XCTAssertTrue(antarcticaArea.waitForExistence(timeout: kTimeOut))
        commas = antarcticaArea.label.filter({return $0 == ","}).count
        XCTAssertEqual(2, commas)
    }
}
